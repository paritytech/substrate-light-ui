...
import { DerivedBalancesAll, DerivedStakingAccount, DerivedUnlocking } from '@polkadot/api-derive/types';
import { formatBalance, formatNumber } from '@polkadot/util';
import React from 'react';
import Loader from 'semantic-ui-react/dist/commonjs/elements/Loader';

import { Icon, WrapperDiv } from './index';
import { DynamicSizeText, FadedText, Stacked, StackedHorizontal, StyledLinkButton } from './Shared.styles';
...
export function BalanceDisplay(props: BalanceDisplayProps = defaultProps): React.ReactElement {
  const { allBalances, allStaking, detailed, fontSize, fontWeight, handleRedeem, orientation } = props;

  const renderRedeemButton = (): React.ReactElement | null => {
    return allStaking && allStaking.controllerId ? (
      <StyledLinkButton
        onClick={(): void =>
          allStaking && allStaking.controllerId && handleRedeem && handleRedeem(allStaking.controllerId.toString())
        }
      >
        <Icon name='lock' />
        Redeem Funds
    ...
    return (
      <>
        ...
        <WrapperDiv width='90%' margin='0' padding='0'>
          ...
          <StackedHorizontal justifyContent='space-between'>
            <b>Reserved:</b>
            {reservedBalance && <FadedText>{formatBalance(reservedBalance)}</FadedText>}
          </StackedHorizontal>
          <StackedHorizontal justifyContent='space-between'>
            <b>Locked:</b>
